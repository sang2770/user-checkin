...
export class IngredientDialogComponent implements OnInit {
  ...
  units = [
    'gram', 'kg', 'lít', 'ml', 'chai', 'lon', 'gói', 'hộp', 
    'quả', 'viên', 'cây', 'miếng', 'phần', 'tô', 'chén'
  ];
  ...
  constructor(
    ...
    this.ingredientForm = this.fb.group({
      ...
      currentStock: [data?.currentStock || 0, [Validators.required, Validators.min(0)]],
      costPrice: [data?.costPrice || 0, [Validators.required, Validators.min(0)]],
      lowStockAlert: [data?.lowStockAlert || 0, [Validators.min(0)]]
    });
  }
