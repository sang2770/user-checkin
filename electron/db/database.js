...
module.exports = {
  ...
  createSaleOrder: (saleOrder, items) => {
    return new Promise((resolve, reject) => {
      db.serialize(() => {
        ...
        // Insert sale order
        db.run(`INSERT INTO sale_orders (name, date, employeeName, totalAmount, note) 
                VALUES (?, ?, ?, ?, ?)`,
          [saleOrder.name || '', saleOrder.date, saleOrder.employeeName, saleOrder.totalAmount, saleOrder.note],
          function (err) {
            if (err) {
              db.run('ROLLBACK');
              return reject(err);
      ...
      try {
        ...
        db.run('BEGIN TRANSACTION', (beginErr) => {
          ...
          const processRow = (index) => {
            ...
            const employee = String(row[columnMap.employee] || 'Unknown').trim();
            ...
            if (!productName || productName.length === 0) {
              errors.push(`Dòng ${index + 2}: Tên sản phẩm không được để trống`);
              processRow(index + 1);
              return;
            }
            ...
            this.findProductByName(productName).then(product => {
              if (!product) {
                ...
                console.log(`Tạo sản phẩm mới: ${productName} với giá ${unitPrice}`);
                this.createProduct(newProduct).then(createdProduct => {
                  this.createImportedSaleOrder(createdProduct, quantity, totalAmount, employee, saleDate)
                    .then(() => {
                      processedCount++;
                      console.log(`Đã tạo order cho sản phẩm mới: ${productName}`);
                      processRow(index + 1);
                    })
                    .catch(err => {
  ...
  createImportedSaleOrder(product, quantity, totalAmount, employee, saleDate) {
    return new Promise((resolve, reject) => {
      ...
      db.run(
        ...
        [`Order Import ${Date.now()}`, formattedDate, employee, totalAmount, note],
        function(err) {
          if (err) {
            reject(err);
            return;
          ...
          db.run(
            ...
            [`Item Import ${Date.now()}`, saleOrderId, product.id, quantity, unitPrice, totalAmount],
            (itemErr) => {
              if (itemErr) {
                reject(itemErr);
                return;
              ...
              module.exports.deductStockByRecipes(product.id, quantity)
  ...
  deductStockByRecipes(productId, quantity) {
    return new Promise((resolve, reject) => {
      ...
      db.all('SELECT ingredientId, quantity FROM recipes WHERE productId = ?', [productId], (err, recipes) => {
        if (err) {
          return reject(err);
        }
