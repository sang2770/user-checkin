...
ipcMain.handle(
  "updateDevice",
  async (_, id, device) => await db.updateDevice(id, device)
);

ipcMain.handle("getDepartments", async () => await db.getDepartments());
ipcMain.handle(
  "addDepartment",
  async (_, code, name) => await db.addDepartment(code, name)
);
ipcMain.handle(
  "deleteDepartment",
  async (_, id) => await db.deleteDepartment(id)
);
ipcMain.handle(
  "updateDepartment",
  async (_, id, code, name) => await db.updateDepartment(id, code, name)
);

ipcMain.handle(
  "getEmployees",
  async (_, filters) => await db.getEmployees(filters)
);
ipcMain.handle(
  "addEmployee",
  async (_, code, name, departmentId, positionId) =>
    await db.addEmployee(code, name, departmentId, positionId)
);
ipcMain.handle("deleteEmployee", async (_, id) => await db.deleteEmployee(id));
ipcMain.handle(
  "updateEmployee",
  async (_, id, code, name, departmentId, positionId) =>
    await db.updateEmployee(id, code, name, departmentId, positionId)
);

ipcMain.handle("getPositions", async () => await db.getPositions());
ipcMain.handle(
  "addPosition",
  async (_, code, name) => await db.addPosition(code, name)
);
ipcMain.handle("deletePosition", async (_, id) => await db.deletePosition(id));
ipcMain.handle(
  "updatePosition",
  async (_, id, code, name) => await db.updatePosition(id, code, name)
);
...
ipcMain.handle("getAttendance", async (_, filters) => {
  ...
  employeeList.forEach((employee) => {
    employee.department = departmentList.find(
      (department) => department.id === employee.departmentId
    );
    employee.position = positionList.find(
...
ipcMain.handle(
  "deleteAttendance",
  async (_, id) => await db.deleteAttendance(id)
);
ipcMain.handle(
  "deleteAttendancesByIds",
  async (_, ids) => await db.deleteAttendancesByIds(ids)
);
ipcMain.handle(
  "updateAttendance",
  async (_, id, attendanceData) => await db.updateAttendance(id, attendanceData)
);
  ...
  for (let i = 0; i < attendanceList.length; i += BATCH_SIZE) {
    ...
    for (const item of batch) {
      ...
      if (!position && item.positionName?.length > 0) {
        const newPosition = await db.addPosition(
          item.positionName,
          item.positionName
        );
...
ipcMain.handle(
  "setSetting",
  async (_, key, value) => await db.setSetting(key, value)
);
...
autoUpdater.on("update-available", () => {
  ...
  dialog
    .showMessageBox({
      ...
      message:
        "Một bản cập nhật mới đã có sẵn. Bạn có muốn cập nhật ngay bây giờ không?",
      buttons: ["Có", "Không"],
    })
    .then((result) => {
...
autoUpdater.on("update-downloaded", () => {
  ...
  dialog
    .showMessageBox({
      ...
      message:
        "Bản cập nhật đã được tải xuống. Bạn có muốn cài đặt ngay bây giờ không?",
      buttons: ["Có", "Không"],
    })
    .then((result) => {
      if (result.response === 0) {
